...
version](#id_#%20Change%20format%20validation%20version) provides
examples how to change manually the version.

## Examples

Using
[`spiges_get_df()`](http://swissstatsr.org/SpiGesXML/reference/spiges_get_df.md),
you can get the data from any SpiGes XML file using the `x` argument.
The `x` argument can be an URL or the path of a give XML file.
...
If for some reasons there is no data for a given node,
[`spiges_get_df()`](http://swissstatsr.org/SpiGesXML/reference/spiges_get_df.md)
...
The
[`spiges_get_df()`](http://swissstatsr.org/SpiGesXML/reference/spiges_get_df.md)
function validate internally the XML file. If correctly validated, it
returns the data with a success message in the console. If the
validation is incorrect, only an error message is returned. To return
the data even with an incorrect validation, you can add use the `force`
argument as `TRUE`.
...
If you use this incorrect XML file,
[`spiges_get_df()`](http://swissstatsr.org/SpiGesXML/reference/spiges_get_df.md)
will return an error message (but the function can return the data
anyway using `force = TRUE`):
...
[`spiges_get_df()`](http://swissstatsr.org/SpiGesXML/reference/spiges_get_df.md).
Under the hood the function is using the `spiges-ids` format validation
(while other available nodes are using the `spiges-data` validation
format).
